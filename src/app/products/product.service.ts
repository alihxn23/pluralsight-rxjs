...
export class ProductService {
  ...
  private productInsertedAction$ = this.productInsertedSubject.asObservable();
  ...
  addProduct(newProduct?: Product): void {
    ...
    //       map((cats) => cats.find((c) => c.id === +newProduct.category))
    //     );
    // this.productCategoryService.productCategories$.pipe(
    //   map(
    //     (cats) =>
    //       ({
    //         ...newProduct,
    //         category: cats.find((c) => c.id === newProduct.categoryId)?.name,
    //       } as Product)
    //   )
    // );
